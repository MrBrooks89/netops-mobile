...
describe('WildcardMaskScreen', () => {
  it('derives the wildcard mask from a netmask by default', async () => {
    const { getAllByText, getByText } = await renderWithApp(<WildcardMaskScreen tool={tool} />);
  ...
  it('accepts a bare prefix', async () => {
    const { getByTestId, getByText } = await renderWithApp(<WildcardMaskScreen tool={tool} />);
  ...
  it('rejects a non-contiguous netmask', async () => {
    const { getByTestId } = await renderWithApp(<WildcardMaskScreen tool={tool} />);
