...
describe('CidrCalculatorScreen', () => {
  it('converts prefix ↔ mask by default', async () => {
    const { getAllByText, getByText } = await render(<CidrCalculatorScreen tool={tool} />);
  ...
  it('splits into N equal subnets', async () => {
    const { getByTestId, getByText } = await render(<CidrCalculatorScreen tool={tool} />);
  ...
  it('rejects a non-power-of-two split count', async () => {
    const { getByTestId } = await render(<CidrCalculatorScreen tool={tool} />);
    await fireEvent.press(getByTestId('mode-split'));
